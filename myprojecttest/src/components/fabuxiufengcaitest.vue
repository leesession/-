<template>
  <div class="fabuxiufengcai">
    ...
    <div class="dongtai " @click="test" id="fbxfcdongtai"> 发布</div>

    <div class="test" style="border: 1px solid seagreen">
      <p v-for="item in base64">{{item}} </p>
    </div>
...
//wx 选择图片 或者拍照
    ...
    methods: {
      ...
      chooseImg: function () {
        ...
        wx.chooseImage({
          ...
          success: function (res) {
            ...
            self.dateBase64ToUrl();

          }
        });
      },//选择图片
      /**
       * 将微信返回的localid，转为base64
       * */
      dateBase64ToUrl:function () {
        var self=this;
        wx.getLocalImgData({
          localId:self.photoList[self.basei],
          success: function (res) {
            var localData = res.localData;//base64
            localData = localData.replace('jgp', 'jpeg');
            self.base64urlarray.push(localData);
            // self.postImg(localData);
            self.basei++;
            if (self.basei < self.photoList.length) {
              setTimeout(function () {
                self.dateBase64ToUrl(self.photoList);
              },100)
            }else{
              alert("开始上传到七牛云");
              self.postImg()
      ...
      postImg() {
        ...
        var pic =self.base64urlarray[self.basearrayi].replace("data:image/jpeg;base64,","");
        alert("pic:"+pic);
        ...
        xhr.onreadystatechange = function () {
          if (xhr.readyState == 4) {
            ...
            if (xhr.status == 200) {
              ...
              self.loading.close();
              alert(JSON.parse(xhr.responseText).hash);
             // self.base64.push(JSON.parse(xhr.responseText).hash);
              self.base64.push("http://p5kgkqzx0.bkt.clouddn.com/" + JSON.parse(xhr.responseText).hash);
              if (self.basearrayi < self.base64urlarray.length) {
                setTimeout(function () {
                  self.postImg();
                },100)
                // alert("basearrayi:"+self.basearrayi);
              }
            } else {
              self.$loading.fullscreenLoading = false;
              self.loading.close();
