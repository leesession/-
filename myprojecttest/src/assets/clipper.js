...
import confirmModify from '../components/gerenzhongxin/xiangxixinxi'
...
export default {
  ...
  install( Vue ){
    ...
    Vue.prototype.postImg = function( imageData ) {
      ...
      imageData=imageData.substring(subLen);
      ...
      let url = "http://upload-z2.qiniu.com/putb64/" + fileSize(imageData);
//此处需要注意，不同的区域配置 （http://upload-z2.qiniu.com）会不同，我的链接为华南的链接
      ...
      xhr.onreadystatechange  = function () {
        if (xhr.readyState == 4) {
          ...
          console.log("http://p5kgkqzx0.bkt.clouddn.com/" + keyText.key);
          confirmModify.methods.confirmModify("","http://p5kgkqzx0.bkt.clouddn.com/" + keyText.key,self);
          self.destoried();
        }
      };
      ...
      xhr.setRequestHeader("Authorization", "UpToken "+self.COMMEURL.token);
      xhr.send(imageData);

      // //这里写接口
      //   self.$axios.post(self.COMMEURL.commonUrl+ '/index.php?s=/ucenter/newindex/portrait',
      //     qs.stringify({
      //       "imgData": imageData,
      //     })
      //   ).then(function (res) {
      ...
      //       console.log(confirmModify);
      //       confirmModify.methods.confirmModify("",res.data.data,self);
      //       self.destoried();
      //     }
      //
      //   }).catch(function (e) {
